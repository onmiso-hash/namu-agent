...
**새 작업이 들어갈 프로젝트는 규칙으로 정해진다** — 내 PC에서는 `project`를 생략하면
지금 열려 있는 폴더에 만들어지고, 없는 이름을 직접 적어 주면 거절된다(새 프로젝트는
폴더를 여는 사람만 만든다). 웹에서는 이미 있는 방이나 `web-project`를 골라 줘야 하고,
안 정해졌으면 만들지 않고 방 목록이 돌아온다 — 그 목록을 사용자에게 그대로 보여주고
고른 이름으로 다시 부른다. 어느 쪽이든 새 프로젝트는 생기지 않는다. 실사고: 웹채팅이 확인 없이 "onnamu-security" 같은
프로젝트 이름을 그 자리에서 지어냈고, 확인 칸·질문·시간 문턱으로 검사하는 방식은
두 번 다 뚫렸다 — 지금은 이름을 적어 넣을 자리 자체가 없다.
