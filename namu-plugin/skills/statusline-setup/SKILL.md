...
Claude Code, agy(Antigravity CLI), Grok을 지원한다 — 스크립트가 등록부를
자동으로 훑어 **설치가 감지된 호스트만** 셋업한다. 호출 측에서 호스트를 지정할 필요는 없다.
Grok은 `~/.grok/config.toml`의 `[ui.status_line]`에 붙인다.
...
출력은 감지된 호스트마다 `[claude]`/`[agy]`/`[grok]` 라벨을 붙여 한 줄씩 나온다. 여러 호스트가
...
- **성공(exit 0)** — 출력된 안내(신규 설정/자동 갱신/변경 없음)를 그대로 사용자에게
  ...
  "Grok CLI를 재시작하면 반영됩니다."처럼 대상 호스트에 맞춰 출력한다).
- **양쪽 다 미설치 오류(exit≠0, "설치돼 있지 않습니다")** — 출력된 안내를 그대로 보여주고
  멈춘다. 재실행 전에 플러그인 설치부터 안내한다(Claude Code, agy, Grok 중 하나는 설치돼
  있어야 한다).
- **타 statusLine 감지로 거부(exit≠0, "이미 다른 statusLine 설정이 있습니다")** — 어느
  호스트에서 거부됐는지(`[claude]`/`[agy]`/`[grok]` 라벨) 확인하고, 출력에 포함된 기존 설정 내용을
