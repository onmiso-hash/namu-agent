...
def tool_description() -> str:
    ...
    **순서가 중요하다.** 붙는 쪽(MCP 클라이언트)은 도구 설명을 앞에서부터 잘라 쓰고,
    ...
    lines = [
        ...
        "브리핑에 실리지 않아 길이 제한이 없다). **\"경위는 그 기록에 있다\"고 "
        "가리키는 문장은 직접 쓰지 않는다** — 나무가 브리핑에 `상세:` 줄로 붙여 주므로, "
        "직접 쓰면 그 글자만큼 요약이 쓸 자리가 줄어든다.",
...
_TOOL_LINES = (
    ...
    ("namu_check_ticket", "올리기 링크로 파일이 도착했는지 본다 — 회원이 '올렸어'라고 "
                          "했을 때 확인하는 용도."),
    ("namu_record_session", "이 대화를 재서 남긴다 — 대화를 마칠 때 한 번 부른다. "
                            "회원이 한 말을 원문 그대로 넘기면 서버가 어긋난 자리를 "
                            "센다. 판정은 서버가 하니 숫자를 지어내지 말 것. 클로드 "
                            "코드에서는 훅이 알아서 하므로 부를 필요가 없다."),
)
