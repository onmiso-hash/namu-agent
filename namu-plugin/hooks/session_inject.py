...
def _ensure_db(cfg) -> None:
    """다섯 그릇의 검색 색인을 세션 시작 시점에 맞춘다(session_recall.py와 같은 이유).

    외부 터미널에서 git pull 후 agy를 시작하면 db가 낡아 있을 수 있다(07-10 실측:
    yaml 40건 vs db 37건). 교훈만 재생성하던 것을 `ensure_indexes` 한 번으로 넓혔다
    (fts5-memo-tasks-index 4단계). 안 낡았으면 stat 몇 번으로 끝난다.
    """
    import db
