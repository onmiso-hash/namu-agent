...
동작: 사용자의 마지막 말이 마무리 신호일 때만 개입한다. 이번 세션에 이 프로젝트의
...
def _lines_since(project: str, since_ts: str) -> tuple[list[str], list[str]]:
    """(이번 세션에 줄이 들어간 task 슬러그, 그중 [다음]류가 들어간 슬러그).
    ...
    touched: list[str] = []
    satisfied: list[str] = []
    for entry in task_resolve.journal(project=project, since=since_ts):
        slug = entry.get("task_slug") or ""
        if slug and slug not in touched:
            touched.append(slug)
        if entry.get("tag") in _SATISFYING_TAGS and slug and slug not in satisfied:
            satisfied.append(slug)
...
def _block_reason(project: str, touched: list[str]) -> str:
    ...
    if touched:
        ...
        body = (
            f"이번 세션에 기록이 들어간 task: {targets}\n"
            "지금 남기세요 — `namu_record(bowl='tasks', "
            f"project='{project}', task='<위 task 중 하나>', tag='다음', "
