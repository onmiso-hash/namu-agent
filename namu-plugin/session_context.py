...
# `다음:` 칸에는 요약만 들어간다(mcp_server.NEXT_LINE_LIMIT이 300자로 막는다). 경위는
# 같은 작업일지의 `상세` 칸에 들어가고, 브리핑은 한 줄 요약만 싣는다. 그러니 "이어서
# 하자"는 말을 받았을 때 이 칸만 읽고 착수하면 기준선·측정값·미결 사항을 모르는 채로
# 시작한다. 2026-09-10 사용자 요청으로 읽기를 먼저 지시한다.
#
# 처음에는 경위를 작업 폴더 안의 파일로 옮기게 했는데 그 판을 걷었다 — 웹에서 나무를
# 쓰면 그 파일을 열 방법이 없다(검색 색인은 log.md와 task.md만 담고, 웹 도구에는
# 작업 폴더의 파일을 읽는 것이 없다). `상세` 칸은 브리핑에 안 실리면서 검색으로는
# 그대로 꺼내지므로, 파일을 만들 이유 자체가 없었다.
_RESUME_READ_FIRST_NOTE = (
    "`다음:` 칸에는 **요약만** 들어 있습니다(300자 상한). 경위·측정값·미결 사항은 그 "
    "task의 작업일지 `상세` 칸에 있고 브리핑에는 실리지 않으니, **착수하기 전에** "
    "`namu_search(bowl='tasks', task='<task 이름>')`으로 그 task의 최근 기록을 "
    "읽으세요."
)
...
def _open_task_explain_lines(missing_next: int, pinned: bool = False) -> list[str]:
    ...
    `다음:` 칸은 300자 상한(mcp_server.NEXT_LINE_LIMIT)이 걸려 요약만 실린다. 경위는
    같은 작업일지의 `상세` 칸에 있고 브리핑에는 실리지 않으므로, 그 칸만 읽고 착수하면
    기준선과 측정값을 모른 채 시작한다 — 먼저 읽으라는 지시를 함께 낸다(2026-09-10
    사용자 요청). 상세는 _RESUME_READ_FIRST_NOTE 선언부에 적었다.
