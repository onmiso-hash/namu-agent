...
# 71줄 4,041자 중 30줄 1,550자를 혼자 차지했다. 사용자 지적으로 요약 한두 줄로
# 바꾸자 브리핑이 44줄 2,674자가 됐다. 밀어낸 경위는 같은 task의 `기록` 한 건의
# body 칸으로 간다 — body는 브리핑에 안 실리면서 검색으로는 그대로 꺼내진다.
...
def _validate_task_tag_text(tag: str | None, text: str | None) -> tuple[str, str]:
    ...
    if tag == _NEXT_TAG and len(text) > NEXT_LINE_LIMIT:
        raise ValueError(
            ...
            "적고, 그날의 경위·측정값·설계 내용은 같은 task에 status='기록'으로 한 건 "
            "더 남겨 그 body 칸에 넣으세요 — body는 브리핑에 실리지 않고 namu_search로 "
            "꺼내므로 길이 제한이 없고, 다른 PC와 웹에서도 그대로 읽힙니다. "
