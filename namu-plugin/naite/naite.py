...
def 되돌림_찾기(세션, 넓게=False):
    ...
    for 차례, (시각, 글) in enumerate(세션["발화"]):
        if 차례 == 0:
...
def 날짜만(시각):
    return 시각[5:10].replace("-", "-") if len(시각) >= 10 else "?"
...
def 출력_요약(모음, 인자):
    ...
    print("  ※ 말로 반박은 낱말 규칙으로 판정하므로 넷 중 하나쯤은 잘못 잡힙니다(실측 23%).")
    print()
    print("  → 낱낱을 보려면   --자세히")
    print()
...
def 명령_되돌림(인자):
    ...
    모음 = []
    읽은세션 = 0
    for 파일 in sorted(기록_뿌리.rglob("*.jsonl")):
        세션 = 세션_읽기(파일)
        if not 세션["발화"] and not 세션["중단"] and not 세션["거절"]:
            continue
        if 인자.방 and 방이름(세션) != 인자.방:
            continue
        읽은세션 += 1
        for 건 in 되돌림_찾기(세션, 넓게=인자.넓게):
            ...
            if 자를시각 is not None and t < 자를시각:
                continue
            if 인자.날 and t.astimezone().strftime("%Y-%m-%d") != 인자.날:
                continue
            건["세션"] = 세션이름(세션)
            건["방"] = 방이름(세션)
            건["파일"] = 파일.name
