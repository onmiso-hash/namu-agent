...
-----------
  ...
  B 세션당 구조 표지 건수   요청 중단·도구 거절(사람이 거절한 것)만. 판정이 아니라 표지다
  (A·B의 분모는 사람이 한 마디라도 한 세션 전부다 — `어긋남_재기` 참고)
...
class 어긋남_잰값(NamedTuple):
    건수: int          # 어긋남 전부
    세션수: int        # 분모 — 사람이 한 마디라도 한 세션
    표지: int          # 구조 표지(요청 중단·도구 거절)
    빠짐: int          # 종료 훅이 돌았어야 하는데 안 돈 세션
    옛_세션수: int     # 옛 정의의 분모 — 어긋남이 한 건이라도 있었던 세션
    근거: dict = {}    # 어떻게 쟀나 -> 세션 수 ("대화 기록"·"원문"·"원문·옛 거절"·"저장값")


def 어긋남_재기():
    """어긋남을 센다. `어긋남_잰값`을 돌려준다.

    **분모는 사람이 한 마디라도 한 세션 전부다.** 2026-09-25까지는 어긋남이 한 건이라도
    있었던 세션만 셌는데, 그러면 어긋남 없이 끝난 세션이 분모에서 빠져 "세션당
    어긋남"이 실제보다 높게 나오고, 더 나쁘게는 **어긋남 없는 세션이 늘어도 숫자가
    좋아지지 않는다** — 나아졌는지를 재려고 만든 지표가 나아진 것을 못 본다. 지난
    점검들과 이어 볼 수 있게 옛 정의의 분모(`옛_세션수`)도 함께 돌려준다.
    사람이 한 마디도 안 한 세션은 여전히 빼는데, 그런 세션은 종료 훅도 남기지 않으므로
    (`sessions.measure`) 두 자료의 기준이 같다.
    ...
    **저장된 숫자를 그대로 더하지 않고 지금 규칙으로 다시 잰다**(2026-09-25). 그릇의
    숫자는 남길 때의 규칙으로 잰 값이라, 규칙을 고친 뒤 그대로 더하면 옛 규칙과 새
    규칙이 섞인다 — 실제로 7e5f072e 세션은 저장값 14건이 지금 규칙으로는 3건이었다
    (나머지는 자동 모드 차단). 그릇이 원문을 담는 까닭이 바로 이 재측정이다. 순서는
    이렇다. ① 그릇에 거절 종류까지 있으면 그릇의 원문으로(`원문`). ② 거절 종류가 없는
    옛 항목인데 이 기계에 대화 기록이 남아 있으면 대화 기록으로(`대화 기록`) — 옛
    항목의 거절 목록에는 자동 모드 차단이 섞여 있어 원문만으로는 가를 수 없다.
    ③ 대화 기록도 없으면 그릇의 원문으로 재되 거절은 저장된 대로(`원문·옛 거절`).
    ④ 원문도 없으면 저장값(`저장값`). 어느 쪽을 몇 세션에 썼는지 `근거`로 돌려준다.
    """
    기준 = datetime.now(timezone.utc) - timedelta(days=최근_일수)

    잰것 = {}   # session_id -> (어긋남, 구조표지, 사람 발화 수)
    근거 = {}   # session_id -> 어떻게 쟀나
    기록_파일 = {파일.stem: 파일 for 파일 in naite.대화_기록_파일들()}
    for 항목 in sessions.since(기준):
        세션_id = str(항목["session_id"])
        발화수 = int(항목.get("utterance_count") or len(항목.get("utterances") or []))
        어긋남, 표지, 방식 = sessions.remeasure(항목)
        if 방식 != "원문" and 세션_id in 기록_파일:
            세션 = naite.세션_읽기(기록_파일[세션_id])
            if 세션["발화"]:
                건들 = naite.되돌림_찾기(세션)
                어긋남 = len(건들)
                표지 = sum(1 for 건 in 건들 if 건["갈래"] in ("요청 중단", "도구 거절"))
                방식 = "대화 기록"
        잰것[세션_id] = (어긋남, 표지, 발화수)
        근거[세션_id] = 방식
    측정시작 = 측정_시작_시각()
    ...
    # 서브에이전트의 대화 기록은 뺀다 — 파일 이름을 세션 id로 쓰므로, 섞이면 그 한
    # 장이 세션 하나로 세어진다(naite.대화_기록_파일들 참고).
    for 세션_id, 파일 in sorted(기록_파일.items()):
        if 세션_id in 잰것:
            continue
        세션 = naite.세션_읽기(파일)
        if not 세션["발화"]:
    ...
    전체 = sum(어긋남 for 어긋남, _, _ in 잰것.values())
    표지 = sum(표 for _, 표, _ in 잰것.values())
    세션수 = sum(1 for _, _, 발화수 in 잰것.values() if 발화수 > 0)
    옛_세션수 = sum(1 for 어긋남, _, _ in 잰것.values() if 어긋남)
    근거_셈 = {}
    for 방식 in 근거.values():
        근거_셈[방식] = 근거_셈.get(방식, 0) + 1
    return 어긋남_잰값(전체, 세션수, 표지, 빠짐, 옛_세션수, 근거_셈)


_근거_말 = {
    "대화 기록": "대화 기록을 다시 읽음",
    "원문": "그릇의 원문을 다시 잼",
    "원문·옛 거절": "그릇의 원문을 다시 잼(거절은 저장된 대로)",
    "저장값": "저장된 숫자 그대로",
}
...
def 주된흐름():
    ...
    건수, 세션수, 표지, 빠짐, 옛_세션수, 근거 = 어긋남_재기()
    A = 건수 / 세션수 if 세션수 else 0.0
    B = 표지 / 세션수 if 세션수 else 0.0
    # 옛 정의 — 어긋남이 있었던 세션만 분모로 삼던 값. 지난 점검들과 이어 보려고 둔다.
    옛_A = 건수 / 옛_세션수 if 옛_세션수 else 0.0

    # 분모 정의가 바뀐 뒤 첫 점검이면, 지난 기록의 "세션당 어긋남"은 옛 정의로 잰
    # 값이다. 새 값과 견주면 정의가 바뀐 것만으로 크게 좋아진 것처럼 보이므로,
    # 그때는 옛 정의끼리만 견준다. 지난 기록에 옛 정의 줄이 있으면 새 정의 기록이다.
    지난_옛 = 지난값(지난, "세션당 어긋남(옛 정의)")
    정의_바뀐_첫_점검 = 지난 is not None and 지난_옛 is None
    if 정의_바뀐_첫_점검:
        지난_옛 = 지난값(지난, "세션당 어긋남")
    교훈수, 실패수, 실패들 = 교훈_재기(docs)
    규칙수, 규칙글자 = 상시규칙_재기()

    오래된 = min((p.stat().st_mtime for p in naite.대화_기록_파일들()),
                default=None)
    ...
    if 정의_바뀐_첫_점검:
        견줌_A = "(분모 정의가 바뀌어 지난 값과 견주지 않음 — 아래 옛 정의 줄로 견준다)"
        견줌_B = "(분모 정의가 바뀌어 지난 값과 견주지 않음)"
    else:
        견줌_A = 화살(A, 지난값(지난, "세션당 어긋남"))
        견줌_B = 화살(B, 지난값(지난, "세션당 구조 표지"))
    print("    세션당 어긋남: %.2f 건   %s" % (A, 견줌_A))
    print("    세션당 구조 표지: %.2f 건   %s" % (B, 견줌_B))
    print("    (어긋남 %d건 / 사람이 말한 세션 %d개, 최근 %d일, 대화 기록 보존 %s)"
          % (건수, 세션수, 최근_일수, 보존))
    print("    옛 정의(어긋남 있던 세션 %d개로 나눔): %.2f 건   %s"
          % (옛_세션수, 옛_A, 화살(옛_A, 지난_옛)))
    if 근거:
        # 지금 규칙으로 다시 잰 것과 저장값을 그대로 쓴 것을 숨기지 않는다.
        차례 = ("대화 기록", "원문", "원문·옛 거절", "저장값")
        print("    (지금 규칙으로 잰 근거: %s)" % " · ".join(
            "%s %d세션" % (_근거_말[방식], 근거[방식]) for 방식 in 차례 if 근거.get(방식)))
        if 근거.get("원문·옛 거절") or 근거.get("저장값"):
            print("    ※ '옛 거절'·'저장값' 세션은 자동 모드 차단이나 옛 규칙이 섞여 있어"
                  " 실제보다 높게 나올 수 있습니다.")
    if 빠짐:
    ...
    print("  ■ 나무에 남길 때 본문에 아래 여섯 줄을 그대로 넣는다")
    print("    (다음 점검이 이 줄을 읽어 대조하고, 조치를 안 남기면 잡아낸다)")
    print()
    print("    세션당 어긋남: %.2f" % A)
    print("    세션당 어긋남(옛 정의): %.2f" % 옛_A)
