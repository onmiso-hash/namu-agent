...
| 파일 | 역할 |
|------|------|
| `mcp_server.py` | FastMCP 메모리 서버. MCP 도구 `namu_recall`/`namu_search`/`namu_record` 노출, stdio 전송 |
| `db.py` | learnings.yaml ↔ SQLite 코어. 읽기(recall/search)는 conn 인자를 받고, 쓰기(record/init_db/rebuild)는 함수 내부에서 conn을 열고 닫는다 |
| `config.py` | 경로·환경변수 일원화. `NAMU_HOME`/`NAMU_MACHINE`·learnings/db/tasks 경로 산출, `load_dotenv` 호출 |
...
- `NAMU_HOME` 환경변수 — learnings/tasks/db 데이터가 저장될 경로(이 repo 절대경로)
- `NAMU_MACHINE` 환경변수 — 현재 PC 식별자(미설정 시 `unknown` 폴백, 상태 파일 매칭이 깨짐)
