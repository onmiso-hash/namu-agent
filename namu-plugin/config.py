...
FIELDS: tuple[Field, ...] = (
    ...
    Field(
        ...
        bowls=("tasks",),
        required_in=(),
        desc="'새 프로젝트를 만들자'를 **사용자가 직접 골랐다**는 뜻이다. AI가 스스로 "
             "판단해 켜는 칸이 아니다. 아직 없는 프로젝트 이름으로 새 작업(create)을 "
             "만들려 하면 첫 호출은 이 값과 상관없이 언제나 거절되고, 사용자에게 그대로 "
             "보여줄 질문(기존 프로젝트 번호 목록 + '새로 만들기')이 돌아온다 — 그 질문을 "
             "보여주고 답을 받은 뒤에 다시 불러야 한다. 사용자가 답할 시간도 없이 곧바로 "
             "다시 부르면 또 거절된다.",
        example="True",
    ),
