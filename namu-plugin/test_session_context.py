...
def test_find_active_skips_completed(monkeypatch, tmp_path):
    ...
    done = _make_task(tmp_path, "done-task", "hp", "(완료)", log_lines=["[완료] 2026-06-29 10:00:00 hp · 완료"])
    active = _make_task(tmp_path, "active-task", "hp", "다음 단계 구현", log_lines=["[시작] 2026-06-28 10:00:00 hp · 시작"])
    result = _sc.find_active_task()
...
def test_find_active_most_recent_in_progress(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_cfg, "TASKS_DIR", tmp_path)
    older = _make_task(tmp_path, "older-task", "hp", "이전 할 일", log_lines=["[시작] 2026-06-28 10:00:00 hp · 시작"])
    newer = _make_task(tmp_path, "newer-task", "hp", "최신 할 일", log_lines=["[시작] 2026-06-29 10:00:00 hp · 시작"])
    result = _sc.find_active_task()
    assert result is not None
...
def test_find_active_all_complete_returns_none(monkeypatch, tmp_path):
    """전부 완료면 None."""
    monkeypatch.setattr(_cfg, "TASKS_DIR", tmp_path)
    _make_task(tmp_path, "done1", "hp", "(완료)", log_lines=["[완료] 2026-06-29 10:00:00 hp · 완료"])
    _make_task(tmp_path, "done2", "hp", "(완료)", log_lines=["[완료] 2026-06-29 10:00:00 hp · 완료"])
    assert _sc.find_active_task() is None

def test_build_markdown_has_task_and_learnings(monkeypatch, tmp_path):
    """진행 중 task + 교훈 → '📌 진행 중'과 '💡' 둘 다 포함."""
    monkeypatch.setattr(_cfg, "TASKS_DIR", tmp_path)
    _make_task(
        tmp_path, "my-task", "hp", "헬퍼 통합 구현",
        log_lines=[
    ...
    md = _sc.build_context_markdown(conn, "hp")
    conn.close()
...
def test_build_markdown_empty_returns_none(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_cfg, "TASKS_DIR", tmp_path)
    conn = _setup_mem_db([])
    result = _sc.build_context_markdown(conn, "hp")
    conn.close()
