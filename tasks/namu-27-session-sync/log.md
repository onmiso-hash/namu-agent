...
[완료] 2026-07-10 21:47:12 samsung · #27 이월 목록 정정 — ctx% '?' 폴백은 결함 아님으로 판정해 후보에서 제외. 이월: 기저 테스트 실패 6건 정리(test_cache_stale Windows temp·test_mcp_selfheal cp949 캡처)·statusLine 제목 중복 표시 cosmetic. 제외 근거: statusline.log 409렌더 실측상 '?'는 세션 첫 렌더에서만 발생(CC가 그 시점 context_window.used_percentage를 미제공)하고 다음 렌더에 자가 복구 — namu_statusline.py:76의 의도된 정직 폴백이라 고칠 대상 아님
