...
## 완료조건
- [ ] ① config.py 파일명 분기 — NAMU_HOME == REPO_ROOT(개발 모드)일 때만
...
- [ ] ② 문서화 — CLAUDE.md "메모리 구조"에 3원 분류(제품지식=repo
...
- [ ] ③ ~/.namu 동기화 켜기 — 사용자가 비공개 원격 repo 준비 후
      `namu_sync_setup` 호출로 활성화. 기존 ~/.namu의 교훈(1건)이 원격에
      push되고 `.namu_sync` 마커 생성 확인.
- [ ] ④ 검증 — 기존 테스트 전수 통과 + 개발 repo 세션에서 recall/record가
