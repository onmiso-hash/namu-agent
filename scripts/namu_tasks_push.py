...
⚠️ 이 repo 루트 사본은 설치형 사용자용 동봉 사본(`namu-plugin/scripts/namu_tasks_push.py`)과
쌍을 이룬다(namu-26 statusline 동봉 관례와 동일) — import 경로 계산만 다르고 나머지
로직은 동일해야 한다. 한쪽을 고치면 반드시 다른 쪽도 맞출 것.
...
# namu-plugin을 path에 추가 — memory_sync는 stdlib only이므로 plain python3로 import 가능
