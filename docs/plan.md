...
> 📅 시작: 2026-06-22 | 최종 갱신: 2026-06-26 (로드맵 14번 **SQLite 캐시 자동 재생성 완료** — 부팅 시 카운트 비교로 stale 판정→rebuild, 커밋 `7e81906`. 교훈+상태 두 메모리 그릇 + pull 동기화까지 갖춤. 다음=15(삼성 동기화·캐시설치 실검증)/16(agy 글루)/CLAUDE.md stale 점검) | 대화를 통해 점진적으로 채워나가는 문서
...
| 2026-06-26 | **14번 트리거 위치** | `_ensure_db()`(MCP 서버 부팅)서 판정. 세션마다 stdio 서버 재기동 → **pull 직후 첫 세션이 곧 트리거**(별도 명령·git 훅 불필요, 제로 셋업). 세션 도중 pull은 못 잡음(서버 재시작 전) — 흔한 워크플로 아니라 MVP 허용, 필요시 수동 rebuild 도구는 후속. rebuild가 내부서 `unlink→init_db`로 스키마 재생성하므로 stale 경로는 rebuild 단독 호출로 충분 |
...
15. 🔄 **삼성 노트북 동기화 검증 — 진행 중 (2026-06-26), 윈도우 버그로 보류**
    - ✅ 삼성 환경점검 전부 통과(Python 3.12.3/SQLite 3.45.1+FTS5/uv 0.11.24/NAMU_MACHINE=samsung/Claude Code 2.1.186)
    - ✅ 삼성 pull 깔끔한 fast-forward(`3aba383..f39e49a`) — namu-plugin/·docs/·.claude/·learnings.yaml(77줄) 도착. CLAUDE.md 재작성본 동기화 확인. `tasks/` gitignore 아님 확인(빈 건 task 미생성일 뿐)
    - 🐛 **#14 첫 실전검증서 윈도우 파일잠금 버그 발견:** `rebuild_from_yaml`의 `db.py:114 unlink(missing_ok=True)`가 **열린 db 핸들 때문에 `WinError 32`**(리눅스는 OK, 윈도우 거부). → HP서 수정 필요(후보 B: 파일삭제 제거, 단일 conn 내 DROP/DELETE 후 재생성)
    - ⬜ HP 수정 → 삼성 ⑤ 재검증(yaml `^id:` 수 = db COUNT 일치) → #14·#15 완료
    - ⬜ 잔여: 진짜 플러그인 캐시 설치(`/plugin install`) 실검증(=13번 미검증 잔여), 버그 해결 후 진행
