...
16. 🟢 **(핵심 목표 + 세션 브리핑 달성) 양쪽 1:1 — statusLine + `/namu`** (statusLine[상시 한 줄]·`/namu`[읽기전용 세션 브리핑] 둘 다 Claude Code+agy 라이브 1:1 통과·커밋·푸시 완료 / 차기=agy 플러그인 승격·서브에이전트 대칭·잔재 청소·선정로직 통합 검토) (+ namu-plugin/agents/ 이동)
