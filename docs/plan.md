...
└── memory/         ← 공통 학습 기억 (git 동기화)
    └── learnings.md  ← append-only, 모든 PC가 추가만 가능
```

**사용 흐름:** git pull → 작업 실행 → git push
**learnings.md 원칙:** 추가만 가능, 삭제/수정 금지 (자동학습 이력 보존)
...
- 동기화: GitHub (memory/learnings.md)
...
├── memory/
│   └── learnings.md           ← 공용 append-only 기억 (git 동기화) ✅
...
## 🧠 메모리 구조 (확정)
```
[공유] memory/learnings.md  ← git으로 PC간 동기화 (진짜 기억, append-only)
           ↓ pull 후 자동 변환 (미구현)
[로컬] db/namu.db           ← 빠른 검색/패턴분석용 SQLite (캐시, gitignore)
```
- learnings.md: 모든 PC가 추가만 가능한 공용 메모장
- SQLite: learnings.md를 구조화해서 패턴분석에 쓰는 로컬 캐시
- git pull 시 learnings.md 최신화 → SQLite 자동 재생성 (구현 예정)
...
10. 🔶 **MCP 메모리 서버 구현** — `namu_recall`/`namu_record`/`namu_search`
    - ✅ 설계 확정 (스키마·SQLite·FTS, mcp_memory_design.md 참조)
    - ⬜ 구현 (HP Claude Code) — db.py(스키마+record/recall/search/rebuild) → mcp_server.py(FastMCP 도구 3개)
    - ⬜ MCP Inspector 테스트 → Claude Code stdio 등록
    - ⬜ learnings.md 포맷 설계 (record↔rebuild 짝, 다음 세션 후보)
