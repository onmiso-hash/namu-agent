...
16. 🔶 **(진행) 양쪽 1:1 자동주입 — Claude Code+agy 통일** (구현 완료·테스트 6/6 / 토대 복구 완료·자동주입 작동 / 남은 진범=플러그인 모드 `NAMU_MACHINE=unknown`) (+ namu-plugin/agents/ 이동)
    ...
    - ⬜ **다음(차기 창 — env 확정부터):** ① 셸 env 실태(`env | grep NAMU` + rc 파일) 확인 ② 모순 해명(교훈 떴는데 machine unknown인 이유 = 실제 자동주입 셸 env 직접 찍기) ③ 유력 해법=HP rc에 `export NAMU_HOME="$HOME/project/namu-agent"`+`export NAMU_MACHINE=hp`(삼성 성공 이유 추정)→새 셸→새 세션→`context.hp.md` 잡혀 📌진행중 뜨는지 ④ 대안=config.py가 repo 루트 .env 폴백(단 캐시 모드 한계, 셸 export가 정석) ⑤ 통과 시 클로드코드 검증→agy 재시작·새 세션 검증→양쪽 1:1 본문 대조→4파일 커밋 + 스모크 잔재(`.agents/hooks/smoke_*`·`agy_*.log`·임시 진단줄) 청소. 이후 3단계=agy 플러그인 승격·agents 이동, 4단계=네이티브 서브에이전트 대칭
