...
16. 🔶 **(진행) 양쪽 1:1 자동주입 — Claude Code+agy 통일** (구현 완료·테스트 6/6 / 토대 복구 완료·자동주입 작동 / 남은 진범=플러그인 모드 `NAMU_MACHINE=unknown`) (+ namu-plugin/agents/ 이동)
    ...
    - ⬜ **다음(차기 창 — 화면 가시성 확보):** ① **실험 = `additionalContext` JSON 벗고 `print(md)` 직접 stdout** → `marketplace update`→`reload`→새 세션서 화면에 뜨나 관측(기존 JSON줄은 주석처리로 보존). ② 안 뜨면(2.1.0이 stdout도 억제) 대안 = **statusLine 표시** / **UserPromptSubmit 훅으로 첫 턴에 노출** / **CLAUDE.md에 "세션 첫 응답서 진행 task 요약 출력" 지시**(셋 다 화면 경로). ③ 가시성 확보되면 클로드코드 라이브 통과→agy 검증→양쪽 1:1 대조→4파일 커밋 + DEBUG 블록·스모크 잔재(`.agents/hooks/smoke_*`·`agy_*.log`) 청소. 이후 3단계=agy 플러그인 승격, 4단계=네이티브 서브에이전트 대칭. ⚠️ DEBUG 블록(`_build_debug_block` + 호출 1줄)은 작업폴더 미커밋 상태 — 가시성 해결 후 함께 제거.
