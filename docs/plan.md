...
> 📅 시작: 2026-06-22 | 최종 갱신: 2026-06-26 (상태 메모리 설계 확정 — `context.md` 되살리기, 구현 대기. 13/16 보류) | 대화를 통해 점진적으로 채워나가는 문서
...
### ▶️ 다음 작업 — 상태 메모리(`context.md`) 되살리기 [13·16보다 먼저]
> 06-26 설계 완료, 구현 대기. 메모리의 빠진 한 층(상태)을 채운다. (상세 = 결정 테이블 06-26 행 + 저녁 세션 로그)
- ⬜ **3파일 템플릿 확정** — `task.md`(목적·완료조건) / `context.<machine>.md`(현재상태 스냅샷) / `log.md`(시간순 이력, machine 도장)
- ⬜ **재진입용 작업 이름(slug) 방식** 결정 (ULID는 사람이 못 찾아 부적합)
- ⬜ **`/namu-task` SKILL.md 개정** — 교훈 북엔드(recall/record) 양옆에 상태 북엔드 추가(시작=재정박 / 진행중=context 덮어쓰기+log append / 끝=log 마침줄+완료시 context 비우기)
- ⬜ **`tasks/` git 추적 전환** (현재 gitignore → 다중 PC 공유의 전제)
- ⬜ (후속) plan.md 프로젝트단위 자동화 — task 기계 검증 후 얹기

13. ⬜ 전체를 Claude Code 플러그인으로 패키징 **(보류 — 상태 메모리 먼저)**
