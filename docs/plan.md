...
> 📅 시작: 2026-06-22 | 최종 갱신: 2026-06-27 (**16번 2단계 설계 완전 확정(아직 미구현)**. v7 미확정 3개를 공식 docs + HP 스모크 실측으로 전부 해소: ① **ephemeralMessage 출력 형식** = `{"injectSteps":[{"ephemeralMessage":"문자열"}]}`(injectSteps 원소는 toolCall/userMessage/ephemeralMessage 중 하나, 우리는 ephemeralMessage 문자열 하나에 상태+교훈 통째 조립) ② **hooks.json 포맷** = **네임스페이스 래핑** `{"namu-inject":{"PreInvocation":[...]}}`(공식 예시로 확정, v7 의문 종결, `.agents/hooks.json`) ③ **세션 1회 플래그 키 = conversationId 확정** — 스모크 실측서 `invocationNum`이 **매 turn 0으로 리셋**돼 첫호출 판정 불가 판명(폐기), `initialNumSteps`도 예측불가라 부적합. `conversationId`만 세션단위 고정+세션바뀌면 변경이라 적격. 플래그 저장=`tempfile.gettempdir()/namu_injected_{conversationId}`(WSL=/tmp·Win=%TEMP% 자동분기). 부수교훈=agy 훅 command는 **cwd 보장 안돼 절대경로 필수**(상대경로면 조용히 죽음). 다음=구현 지시서 작성→HP Claude Code 인계) | 대화를 통해 점진적으로 채워나가는 문서
...
16. 🔶 **(착수) agy용 글루 — Antigravity 방식 자동 주입 + 저장** (+ namu-plugin/agents/ 이동)
    ...
    - ✅ **2단계 설계 완전 확정(2026-06-27, 미구현):** **꺼내기=PreInvocation 훅 + ephemeralMessage**(세션 1회 플래그로 비용 0), **넣기=Claude Code 정책 대칭**(AI 수동 호출, 자동저장 보류), **AGENTS.md 안 건드림**(저장 규칙은 주입 쪽지 동봉), 주입 내용=상태(tasks/) 메인+관련 교훈. v7 미확정 3개 해소(공식 docs+HP 스모크 실측): 출력형식 `{"injectSteps":[{"ephemeralMessage":"..."}]}` / hooks.json=네임스페이스 래핑 / 플래그 키=`conversationId`(invocationNum은 매 turn 0 리셋이라 폐기, 판정=플래그 파일 존재여부), 저장=`tempfile.gettempdir()/namu_injected_{cid}`. 부수교훈=훅 command 절대경로 필수. 레퍼런스=ContextOS. (상세=결정 테이블/세션 로그 06-27)
    - ⬜ **다음:** **구현 지시서 작성(Claude Code 인계)** → `hooks/session_inject.py`(가칭: db.recall 재사용+상태 읽기, ephemeralMessage 한 덩어리 조립, conversationId 플래그, 어떤 예외든 exit 0) + `.agents/hooks.json` 네임스페이스 래핑 등록 → 라이브 검증(agy 새 세션서 상태+교훈 자동 뜨는지) → 스모크 잔재(`smoke_inject.py`·`smoke_recall.py`) 정리. 이후 3단계=agy 플러그인 승격(`~/.gemini/antigravity-cli/plugins/namu/`)·agents 이동, 4단계=네이티브 서브에이전트 대칭
