...
> 📅 시작: 2026-06-22 | 최종 갱신: 2026-06-27 (**13번 완전 종료** — 삼성서 `/plugin install` 플러그인 실설치 검증 통과. `${CLAUDE_PLUGIN_ROOT}` 자동채움+uv 의존성 자급+셸 env 상속+코드/데이터(NAMU_HOME) 분리 모두 실작동. 검증이 marketplace.json 스키마 버그(name/owner 누락) 1건 잡아냄. record가 올바른 NAMU_HOME에 떨어짐 확인(**B 버그 최종 박멸**, 유령경로=False). 삼성→push→HP pull→recall 멀티 PC 동기화 한 바퀴 완주. **16번 진행 중**(agy 글루): 버그 3개 재검증 완료(훅 #49 수정 / MCP env var·non-TTY #76 잔존이나 MVP 무관), **agy MCP 호스트 1단계 검증 통과**(워크스페이스 `.agents/mcp_config.json`, 샌드박스 OFF·env 리터럴 주입·`machine=hp` 확인). 다음=SessionStart 훅→플러그인 승격) | 대화를 통해 점진적으로 채워나가는 문서
...
| 2026-06-27 | **16번 agy 버그 3종 재검증** | 착수 전 웹검색 재확인. ① MCP env var 버그(전역 MCP 설정서 키 하드코딩 강제)=**잔존**(6/22 글) ② non-TTY stdout #76(`agy -p`가 파이프/서브프로세스서 stdout 조용히 누락, exit 0)=**잔존**(pseudo-TTY·transcript 우회 성행) ③ 훅 경로 버그 #49(hooks.json을 `antigravity-cli/` 대신 `config/`에 잘못 씀)=**수정됨**(공식 CHANGELOG). **결론: MVP(MCP호스트+훅+네이티브 서브에이전트)는 초록불.** #76은 후순위로 미룬 subprocess 워커 어댑터에만 걸림. 부수발견=`agy -p` 모델 Flash 하드코딩·`GEMINI_API_KEY` 무시(`ANTIGRAVITY_TOKEN`)·쿼터 병렬소진(Pro도 3~5개 한계)·PreToolUse non-zero=deny(NAMU exit0 설계 정합)·agy 플러그인 구조 Claude Code와 1:1 |
| 2026-06-27 | **16번 MCP 호스트 설계 = env 버그 무관** | agy env 버그는 "설정파일 `${...}` 치환 실패"인데, NAMU는 Claude Code 때 같은 부류 함정 3개를 **셸 export+절대경로**로 이미 우회한 설계라 버그가 물 자리 없음. 포팅=① 스크립트 경로 `${CLAUDE_PLUGIN_ROOT}` 대신 **절대경로** ② NAMU_HOME/NAMU_MACHINE은 `mcp_config.json` `env` 블록에 **리터럴 하드코딩**(버그는 치환만 막지 리터럴은 먹음, 비밀도 아니고 PC별 로컬이라 자연스러움). 배치=NAMU 자기 패턴(로컬 먼저→플러그인)대로 1단계 워크스페이스 `.agents/mcp_config.json`(PC별 절대경로라 gitignore)→2단계 플러그인 승격 |
| 2026-06-27 | **16번 MCP 호스트 1단계 검증 통과** | HP서 `.agents/mcp_config.json`(uv 절대경로+mcp_server.py 절대경로+env 리터럴) 생성→agy `/mcp`에 **`namu-memory ✓connected · 3 tools`**. 리트머스 3개 다 초록: ① recall=NAMU_HOME yaml 정독 ② record 테스트1건이 learnings.yaml **실제 append**(=샌드박스 안 막음)+**`machine: hp`** 박힘(=env 주입 성공. NAMU_HOME은 폴백이 마침 repo루트라 가려질 수 있어 **machine으로 판정**) ③ settings.json에 `enableTerminalSandbox` 키 없음=**샌드박스 OFF**. `.agents/`는 gitignore. `/mcp`가 "Plugins(...)" 헤더 아래 표시했으나 디스크 확인=plugins 폴더 빔→**워크스페이스 로드 확정**(헤더는 라벨일 뿐) |
| 2026-06-27 | agy 권한 posture 헤드업 | HP agy `toolPermission: "always-proceed"`+`trustedWorkspaces`에 namu-agent=도구 호출(파일쓰기·명령) 프롬프트 없이 자동 진행. NAMU 작업규칙 "자율 커밋 금지/매번 Yes"와 결 다름 → **agy 서브에이전트가 커밋까지 자율 가능**. 당장 문제 아니나 워커(#16 후반) 게이트 설계 시 재논의 |
...
### 2026-06-27 (로드맵 16번 착수 — agy 버그 재검증 + MCP 호스트 1단계 검증, 대화창+HP)
- **agy 버그 3종 재검증(대화창, 웹검색):** 착수 전제. ① **MCP env var 버그**(전역 MCP 설정서 키 하드코딩 강제)=**잔존**(6/22 글 명시) ② **non-TTY stdout #76**(`agy -p`가 파이프/리다이렉트/서브프로세스서 모델 왕복 끝나도 stdout 조용히 누락, exit 0)=**잔존**(1~2주 전 글들 pseudo-TTY `script -qec`·transcript.jsonl 읽기로 우회, 전용 MCP 브릿지까지 등장) ③ **훅 경로 #49**(`/hooks`가 `~/.gemini/antigravity-cli/hooks.json`에 잘못 써 안 먹던 버그)=**수정됨**(공식 CHANGELOG: `~/.gemini/config/hooks.json`로 교정). **판정: 우리 MVP(MCP호스트+훅+네이티브 서브에이전트)는 초록불** — #76은 우리가 후순위로 미뤄둔 "이종 엔진 subprocess 워커 어댑터"에만 걸림(MVP는 `/agents` 네이티브라 무관)
- **부수 발견(메모):** `agy -p`는 print 모델 Gemini 3.5 Flash 하드코딩(env/settings override 불가) / `GEMINI_API_KEY` 무시(로컬=agy auth 토큰, CI=`ANTIGRAVITY_TOKEN`) / 서브에이전트 쿼터 병렬 소진(Pro도 3~5개 한계, 2프롬프트만에 잠긴 사례) / PreToolUse non-zero=deny(NAMU "무조건 exit 0" 설계가 정합) / agy 플러그인 구조(`plugin.json`+`mcp_config.json`+`hooks.json`+`skills/`+`agents/`+`rules/`)가 Claude Code와 거의 1:1
- **MCP 호스트 설계 확정(대화창):** agy env 버그는 정확히 "설정파일 `${...}` 치환 실패"인데, NAMU는 Claude Code 때 같은 부류 함정 3개(#9427 등)를 **셸 export+절대경로**로 이미 우회한 설계라 버그가 물 자리 없음. 포팅 = 스크립트 경로 절대경로 + NAMU_HOME/NAMU_MACHINE은 `env` 블록에 리터럴 하드코딩. 배치는 NAMU 패턴대로 **1단계 워크스페이스 `.agents/` → 2단계 플러그인 승격**
- **1단계 실구현·검증(HP, 사용자 직접 스텝바이스텝):**
  - 스텝0 환경값 실측: `pwd`(repo루트=`/home/onmiso/project/namu-agent`)·`realpath namu-plugin/mcp_server.py`·`which uv` 확인
  - 스텝1 `.agents/mcp_config.json` 생성: 쉘이 `$(which uv)`/`$(pwd)`/`$(realpath ...)`를 heredoc서 풀어 **리터럴 절대경로**로 박음(`${...}` 일절 없음) + `env`에 `NAMU_HOME`/`NAMU_MACHINE:hp`
  - 스텝2 `.agents/` gitignore(PC별 절대경로라 로컬 전용)
  - 스텝3 검증: `/mcp` → **`namu-memory ✓connected · Tools: namu_recall, namu_search, namu_record`** / recall 실호출=과거 교훈 정독 / record 테스트1건(`01KW3YFTGDPMMBZ2DQPWZHBM85`, task "agy MCP 연결 1단계 검증", verified_by=human)
  - 스텝4 리트머스: `tail learnings.yaml`로 ⓐ 그 항목 **실제 append 확인**(=샌드박스 안 막음) ⓑ **`machine: hp`** 박힘 확인(=env 주입 성공) / `cat settings.json`에 `enableTerminalSandbox` 키 없음=**샌드박스 OFF**
  - `/mcp`가 "Plugins(~/.gemini/antigravity-cli/plugins)" 헤더 아래 표시해 잠깐 혼동 → `ls`로 디스크 확인: `.agents/mcp_config.json` 존재 + plugins 폴더 빔 → **워크스페이스 로드 확정**(헤더는 그냥 라벨)
- **부수 관측:** HP agy `settings.json`에 `toolPermission: "always-proceed"`+`trustedWorkspaces`에 namu-agent → 도구 호출 자동 진행(프롬프트 없음). NAMU "자율 커밋 금지" 규칙과 결 달라 워커 단계서 게이트 재논의 필요(헤드업)
- **백로그 해소:** 13번서 🔶 미기록이던 "NAMU_HOME 유령경로 함정" 교훈(`01KW3T9T...`, machine: samsung)이 learnings.yaml에 기록된 것 확인
- **의미:** 16번 토대(메모리 코어를 agy에 물림) 완성. env 버그가 NAMU 설계엔 무력함을 실증 — "독립성은 메모리 레이어에" 철학이 두 번째 호스트(agy)서도 그대로 작동. 나머지(훅·서브에이전트)는 이 위에 얹힘
- **다음 세션 시작점:** ① **16번 2단계 SessionStart 훅** — `~/.gemini/config/hooks.json`에 `session_recall.py` 자동 recall 주입, agy 새 세션서 교훈 자동 떠오르는지 로컬 검증(훅 #49 수정됐고 exit0 설계 안전) ② 그다음 3단계 플러그인 승격(MCP+훅+agents 한 묶음, Claude Code #13 대칭) ③ 별건: HP 플러그인 통일
...
16. 🔄 **(진행 중) agy용 글루 — Antigravity 방식 설정/훅 (+ namu-plugin/agents/ 이동)**
    - ✅ **버그 3종 재검증 완료(2026-06-27):** MCP env var=잔존 / non-TTY stdout #76=잔존(but 우리 MVP는 네이티브 서브에이전트라 무관, subprocess 어댑터에만 걸림) / 훅 경로 #49=**수정됨**. → MVP(MCP호스트+훅+네이티브)는 초록불 (상세=결정 테이블/세션 로그 06-27)
    - ✅ **1단계 MCP 호스트 검증 통과(2026-06-27, HP):** 워크스페이스 `.agents/mcp_config.json`(절대경로+env 리터럴)→agy `/mcp` `namu-memory ✓connected·3 tools`. 샌드박스 OFF·record yaml append·`machine=hp` 주입 확인. env 버그는 NAMU의 셸/절대경로 설계로 무관
    - ⬜ **2단계 SessionStart 훅:** `~/.gemini/config/hooks.json`에 자동 recall 주입(NAMU `session_recall.py` 재사용, exit0 설계가 agy PreToolUse=deny 관습과 정합). 로컬 검증 후
    - ⬜ **3단계 플러그인 승격:** MCP+훅+agents를 agy 플러그인(`~/.gemini/antigravity-cli/plugins/namu/`)으로 한 묶음 = Claude Code #13의 agy판 대칭. `namu-plugin/agents/` 이동 동반
    - ⬜ **4단계 네이티브 서브에이전트:** `/agents`로 namu-coder/reviewer 대칭 (12번 워커 구조의 agy판)
    - ℹ️ 사실확인(2026-06-25): agy도 MCP 호스트(`~/.gemini/config/mcp_config.json`·워크스페이스 `.agents/mcp_config.json` + `/mcp`), 훅(`hooks.json`, PreToolUse/PostInvocation/Stop+세션시작), 네이티브 서브에이전트(`/agents`,`agy -p`)
    - 🔶 별건: HP도 플러그인 방식 통일할지 차기 결정
