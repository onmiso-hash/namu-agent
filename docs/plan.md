...
16. 🔶 **(진행) 양쪽 1:1 자동주입 — Claude Code+agy 통일** (구현 완료·테스트 6/6, 라이브 검증은 토대 복구 후) (+ namu-plugin/agents/ 이동)
    ...
    - ⬜ **다음(차기 창 — 토대 복구부터):** ① 검증된 설치명령 확인(삼성 marketplace 흐름 대조: `marketplace add`→`plugin install namu@namu-marketplace`) → HP 플러그인 설치 ② `namu-memory ✓connected` 확인 ③ `settings.local.json` 옛 hooks 줄 제거(permissions는 보존) ④ **라이브 검증**(클로드코드 새세션→agy 새세션→1:1 본문 대조) ⑤ 통과 시 4파일 커밋 + 스모크 잔재(`.agents/hooks/smoke_*`·`agy_*.log`·임시 진단줄) 청소. 이후 3단계=agy 플러그인 승격·agents 이동, 4단계=네이티브 서브에이전트 대칭
