...
| 동작 | **다섯 그릇 모두 SQLite 색인 질의**(fts5-memo-tasks-index). 검색어는 낱말별 **AND**로 나눠 던지고(순서 무관), 낱말이 전부 3자 이상이면 FTS5 trigram MATCH, 하나라도 2자 이하면 색인을 건너뛰고 `LIKE` 전수 조회로 폴백한다(trigram은 두 글자를 원리상 못 찾는다). learnings는 자기 표(`learnings_fts`, bm25 정렬 + outcome 추세 요약), 나머지 넷은 같은 모양의 표 `bowl_<이름>`/`bowl_<이름>_fts`를 쓴다. 원본은 파일 그대로이고 색인은 재생성 가능한 사본이라, 질의 직전에 원본의 크기·수정시각 서명을 보고 바뀐 그릇만 다시 만든다(`db.ensure_bowl_index`). **tasks 그릇에는 두 종류가 담긴다** — `log.md` 한 줄(진행 기록)과 `task.md` 한 장(작업 설명서, 2026-08-08). 칸 구성은 완전히 같고 `tag`가 `설명서`인 것으로 구분한다. 설명서의 시각·기기는 머리말 `📅 생성 …` 줄에서 뽑고, 그 줄이 없는 옛 문서는 그 작업 `log.md`의 가장 이른 줄로 대신한다 |
...
### 5. `namu_task_pin` / `namu_task_unpin` — 책갈피 (namu-70, 2026-08-01)

"다음엔 이것부터"를 정해 두는 도구. **기억을 담는 그릇이 아니라 표시 순서만 바꾼다** —
저장 위치도 네 그릇이 아니라 tasks 폴더 옆의 작은 파일이다.

| 항목 | 내용 |
|------|------|
| 입력 | pin: `task`(str, 필수 — 슬러그 또는 유일 접두, 기록과 같은 해석) / `project`(str, 웹에서는 필수). unpin: `project`만 |
| 출력 | 꽂은/뺀 작업 이름과 기기·시각을 담은 확인 문자열 |
| 저장 | `~/.namu/tasks/<방>/.pin.<machine>` — 1줄 슬러그 + 1줄 시각. 기기당 하나이고 덮어쓴다 |
| 왜 log.md가 아닌가 | ①log는 append-only라 "뗐다 붙였다 하는 현재 상태"를 담을 수 없다 ②log에 적으면 그 task의 `last_ts`가 갱신돼 **순서가 두 경로로 흔들린다** — namu-70이 없애려던 "기록을 건드려 화면 순서를 바꾸는" 짓과 결과가 같아진다. 순서는 표시의 문제이지 기록의 문제가 아니다 |
| 왜 기기마다 파일을 가르나 | 책갈피는 mutable이라 한 파일을 두 PC가 고치면 병합 충돌이 난다(profile.yaml에서 실제로 났던 종류). 각 PC가 제 파일에만 쓰면 충돌 가능성이 0이고, 파일은 그대로 동기화되므로 서로의 책갈피가 보인다(사용자 결정 2026-08-01) |
| 정렬 | 꽂힌 것이 앞, 여러 개면 **최근에 꽂은 순**. 안 꽂힌 것들의 최근 활동순은 그대로(안정 정렬) |
| 닫힌 task | 화면에서 조용히 빠진다(열린 목록과 맞춰 보므로) — 남의 기기 파일을 지우지 않는다. 제 기기 책갈피는 `[완료]`·`[중단]` 기록 시 자동으로 빠진다 |
| 화면 | 브리핑 맨 위 `📌 **slug** … \`hp가 꽂음\``, 그 아래에 **왜 이게 맨 위인지**를 문장으로 밝힌다(완료조건 ①) |
