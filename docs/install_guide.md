...
| **원격(GitHub) 설치** — 캐시 복사 | `claude plugin update namu` (⚠️ 갱신 결과 자체는 실측 대기 — 0.1.6 반영 후 실측 예정, 4-2 참고) | `scripts/agy_reinstall.ps1` 실행(수동: `uninstall → install → --heal`) |
...
- Claude Code `claude plugin update namu`(원격 설치 갱신)의 구체 결과·로그 — 명령어 자체는 확정했으나, 실행 결과(버전 반영 여부 등)는 0.1.6 반영·푸시 후 실측 예정이라 6절에 각주로만 표기했다.
