...
## 1. 개요 — 설치형 vs clone 개발형 vs 셀프호스팅형

NAMU는 세 가지 방식으로 쓸 수 있다.
...
- **셀프호스팅형** — Claude Code/agy 없이 **웹 브라우저의 claude.ai**에서 NAMU 기억(교훈)을 recall/record/search 하고 싶을 때 쓴다. `namu-plugin/http_server.py`를 원격 HTTP 서버로 상시 구동하고 claude.ai Custom Connector로 연결한다. 앞의 두 형태와 달리 자동 세션 브리핑·`/namu-task` 등은 동작하지 않는 등 한계가 뚜렷하다 — 절차·한계 모두 [원격 MCP 셀프호스팅 가이드](remote_mcp_guide.md)를 볼 것.

앞의 두 형태(clone 개발형·설치형)는 같은 메모리 코어(`mcp_server.py`)·같은 워커 정의·같은 오케스트레이션 스킬을 공유한다("봉투 둘, 내용물 하나"). 다른 건 등록 형식(봉투)뿐이다 — **데이터(교훈·작업 상태)가 저장되는 위치는 두 형태가 완전히 동일하다**(namu-35: `NAMU_HOME` 환경변수·"개발 모드/설치 모드" 데이터 분기 자체가 폐지됐다. 상세는 3절 참고). 셀프호스팅형은 이 두 형태와 실행 엔진 자체가 다르므로(CC/agy가 아니라 claude.ai 웹) 별도 문서로 다룬다.
...
> **번외 — clone한 repo를 "로컬 directory 마켓플레이스"로 처음 설정하는 절차**
>
> 1. repo를 clone한다.
> 2. `claude plugin marketplace add /경로/namu-plugin` (또는 CC 대화 세션 안에서 `/plugin marketplace add /경로/namu-plugin`)
> 3. `claude plugin install namu@namu-marketplace`
> 4. 이후 코드 수정은 `git pull` + CC 재시작이면 즉시 반영된다 — 재설치 불필요.
>
> 장점: 개발 중 즉시 반영(소스 갱신만으로 반영, 재설치 없음). 단점: 그 PC에 repo 워킹트리가 실제로 있어야 하고, 아래 4-2 GitHub 원격 방식과 마켓플레이스 이름이 같아 혼동하기 쉽다(4-3절·10절 함정 6 참고).
...
원격 설치는 로컬 directory 소스와 달리 **캐시 복사** 방식이다(`~/.claude/plugins/cache/<marketplace>/<plugin>/<버전>/`에 플러그인 폴더만 복사돼 실행됨). 따라서 소스 갱신만으로는 반영되지 않고 별도 update 절차(8절 참고)가 필요하다.

### 4-3. 로컬 directory 소스 ↔ GitHub 원격 소스 전환 — 실측 절차 (2026-07-13, hp)

4-1의 로컬 directory 방식으로 등록해 둔 마켓플레이스를, 이름은 그대로 두고 4-2의 GitHub 원격 소스로 바꾸는 절차가 실제로 실행·확인됐다(hp = onmiso-hp, WSL bash, CC).

전환 절차(실제 실행):

```
claude plugin marketplace remove namu-marketplace
claude plugin marketplace add onmiso-hash/namu-agent
claude plugin install namu@namu-marketplace
```

- ①은 성공했고, 이때 `settings.json`의 `enabledPlugins`에서 namu 항목이 빠지는 게 관찰됐다.
- ②는 SSH 미설정 환경에서 "SSH not configured, cloning via HTTPS" 메시지 후 자동 HTTPS clone·검증까지 성공했다("Successfully added marketplace: namu-marketplace (declared in user settings)"). 이 과정에서 `settings.json`의 `extraKnownMarketplaces`가 **사용자가 손대지 않아도 자동으로** directory 선언에서 `{source: github, repo: onmiso-hash/namu-agent}` 선언으로 교체된다.
- ③은 기본 user 스코프로 설치됐다(0.1.18 enabled). 전환 후 실행되는 코드는 더 이상 repo 워킹트리가 아니라 `~/.claude/plugins/cache/namu-marketplace/namu/<버전>/`의 **캐시 사본**이다(실측 installPath: `~/.claude/plugins/cache/namu-marketplace/namu/0.1.18/`, `mcp_server.py` 포함).
- 따라서 전환 이후 반영 방법이 바뀐다 — `git pull`이 아니라 `claude plugin update namu@namu-marketplace`가 필요하다(8절 표 참고). statusLine처럼 캐시 경로 안에 버전이 박히는 부수 설정도 같은 이유로 전환 시점에 재설정이 필요하다(실측: `namu_setup_statusline.py` 재실행으로 statusLine 경로가 repo→캐시 사본으로 자동 교체, 백업 생성됨).
- 로컬↔원격은 같은 마켓플레이스 이름(`namu-marketplace`)을 쓰므로, 반대 방향 사고도 가능하다 — 개발 기기에서 실수로 원격 `add`를 하면 로컬 소스가 조용히 원격으로 덮어써진다(10절 함정 6과 동일 현상).
...
플러그인을 지우는 방법도 설치 방식에 따라 다르다.

### Claude Code — 원격(GitHub) 설치 삭제

```
claude plugin uninstall namu@namu-marketplace
```

짧은 이름(`uninstall namu`)이 아니라 **`이름@마켓플레이스` 전체 식별자**를 쓸 것을 권장한다 — 8절 업데이트와 같은 규칙이다. 마켓플레이스 등록 자체까지 지우려면 다음도 실행한다.

```
claude plugin marketplace remove namu-marketplace
```

### Claude Code — 로컬 directory 소스 삭제

위 uninstall + `marketplace remove`에 더해, `settings.json`의 `extraKnownMarketplaces`에 로컬 경로가 하드코딩돼 남아 있으면 그것도 지워야 완전히 삭제된다 — 안 지우면 CC 재시작 시 로컬 소스가 다시 등록된다(2026-07-13, hp 실측 근거).
...
agy의 `plugin install`은 비파괴 병합이라(10절 함정 1), 소스에서 지운 파일이 설치본에 그대로 남는다. 완전 청소에는 uninstall이 필수다(5절 `agy_reinstall.ps1`이 내부에서 이미 이 순서(uninstall → install → --heal)를 따른다).

### 공통 주의 — 데이터는 그대로 남는다

플러그인을 삭제해도 **`~/.namu`의 교훈·작업 상태 데이터는 그대로 남는다** — 데이터 루트가 플러그인 설치 위치와 완전히 독립이기 때문이다(3절 참고). 즉 재설치하면 기억이 그대로 복원된다. 기억까지 지우려면 `~/.namu` 폴더를 별도로 지워야 한다.
...
4. **CC directory 소스는 라이브 참조, 원격 설치는 캐시 복사** — 둘을 혼동해 원격 설치인데 소스만 갱신하고 "왜 반영이 안 되지"라고 헷갈리지 않도록 8절 표를 참고할 것.
