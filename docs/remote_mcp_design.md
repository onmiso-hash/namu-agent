...
> **이 미완 사항이 닫히는 곳(namu-51 설계 확정, 2026-07-26):** `?user=` 라벨을 검증된 신원으로
> 대체하는 작업은 **namu-59-mcp-user-auth**가 맡는다. 그 전제인 신원 계층(GitHub 로그인 +
> GitHub App 설치 + 사용자 ↔ repo 매핑)은 **namu-58-identity-layer**다. 확정된 방식은 아래
> §11-3이며, §11-2는 그 확정에 이르기 전의 스케치로 남긴다.
...
구현은 별도 태스크(사용자별 인증 계층 신설)로 유예된 상태다. → **아래 §11-3으로 확정됨.**

### 11-3. 사용자별 인증 — 확정안 (namu-51 설계, 2026-07-26)

§11-2가 남긴 "토큰 발급 주체 2안"의 답은 **둘 중 하나를 고르는 게 아니라 역할을 나누는 것**이다.

| 무엇을 | 누가 | 왜 |
|--------|------|-----|
| **누구인지 확인** | GitHub 로그인 (외부 IdP 위임, §11-2의 (b)) | 기억 저장소를 사용자 GitHub에 두기로 해(namu-51 결정 1) GitHub App 설치를 위해 어차피 GitHub을 거친다. 신원 확인을 따로 만들 이유가 없다 |
| **MCP 접속 토큰 발급** | 나무 자신 (§11-2의 (a)) | 대시보드(namu-60)가 이미 발급·복사·재발급 창구다. 별도 authorization server를 세울 필요가 없다 |

**핵심은 OAuth 플로우가 아니라 신원의 출처다.** 지금 `?user=`는 요청자가 임의로 적는 URL
쿼리라 위조된다(§11-1). 확정안에서는 그 값이 **서버가 발급하고 서버가 검증하는 토큰**에서
나온다. 라우팅 구조는 그대로다 — 요청 도착 → 토큰 검증 → `user_id` 추출 → `users/<user_id>/`
분기(`_paths_for_user` 재사용). 바뀌는 것은 `user_id`를 **어디서 읽느냐** 한 줄뿐이다.

그래서 **MCP OAuth 2.1을 풀로 구현하지 않아도 된다.** `http_server.py` §5의 기존 인증
미들웨어(헤더 토큰 / 시크릿 경로)를 "고정 토큰 1개" → "사용자별 토큰 N개 조회·검증"으로
확장하는 형태이며, 두 전달 방식(헤더·경로)도 그대로 유지된다. claude.ai 계정에 Request
headers UI가 없는 사용자를 위해 **사용자별 시크릿 경로**가 계속 필요하기 때문이다.

**기억 저장소 접근**(위 접속 인증과 별개 축): 서버는 사용자 GitHub repo에 **GitHub App
installation token**(유효기간 1시간, 사용자가 설치 시 고른 repo 한정)으로 접근한다. 영구
자격증명을 보관하지 않으며, 사용자가 GitHub 설정에서 앱을 제거하면 나무를 거치지 않고 즉시
차단된다. 상세 근거는 `plan.md` 결정 테이블 namu-51 참조.

**남는 한계(안내서에 고지):** 서버는 기억 내용을 평문으로 읽을 수 있다. "남의 GitHub을 못
건드린다"와 "내 기억을 아무도 못 본다"는 다른 보장이며, 후자(암호화 보관)는 검색 캐시와 정면
충돌하므로 이번 설계 범위 밖이다.

**온보딩 확정 (namu-58 실측, 2026-07-26).** 로그인 필수 — **체험판(GitHub 없이 쓰는 나무
서랍)은 폐기**됐다(namu-51 결정 2 개정). 저장소 자동 생성도 불가함이 실측으로 확인됐다:
GitHub App 사용자 토큰으로 `POST /user/repos`를 부르면 `403 Resource not accessible by
integration`이며, 이는 권한 등급 문제가 아니라 개인 계정에 대한 GitHub의 구조적 제약이다
(Administration:write로 뚫리는지는 미검증 — 넣는 즉시 설치 승인 화면에 "저장소 관리
(생성·삭제)"가 영구 노출돼 신뢰지점에 직격하므로 시험하지 않기로 했다).

그래서 온보딩은 이 순서다:

1. **로그인** — GitHub OAuth. 이 화면에는 저장소 권한 문구가 뜨지 않는다(실측: `Verify your
   GitHub identity` / `Know which resources you can access` / `Act on your behalf` 3줄).
2. **저장소가 없으면** `https://github.com/new?name=namu-memory&visibility=private`를 새 탭으로
   연다 — 이름·공개범위가 미리 채워진 상태로 열리므로 사용자는 초록 버튼만 누른다(실측 확인).
3. **앱 설치** — 여기서 비로소 권한 목록(Contents: Read and write)이 뜨고, 사용자가 대상
   저장소를 고른다. 즉 **겁먹을 수 있는 문구는 이 화면에만 나오므로 두 화면을 구분해
   설계**해야 한다.

**서버 사본의 위치(중요).** 연결 사용자의 `users/<user_id>/`는 **사용자 repo의 얕은 복제**이며
나무 소유 STORE 저장소의 git 작업 트리 안에 두어서는 안 된다 — 그렇게 두면 남의 기억이 나무
저장소로 커밋되어 결정 1이 없앤 부담이 되살아난다. 현행 `deploy/entrypoint.sh`의 나무 STORE
clone은 제거 대상이다.

**운영 함정:** 출시 후 앱 권한을 변경하면 **기존 설치자 전원이 새 권한을 수락하기 전까지
403**이 계속된다(커뮤니티 다발 보고). 권한 집합은 출시 전에 확정해야 한다.
